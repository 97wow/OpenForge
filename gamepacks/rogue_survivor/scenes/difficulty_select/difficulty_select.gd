...
func _build_ui() -> void:
	...
	var title := Label.new()
	title.text = I18n.t("SELECT_DIFFICULTY")
	...
	for diff in DIFFICULTIES:
		...
		desc_lbl.text = I18n.t(diff["desc_key"])
		...
		mult_lbl.text = I18n.t("HP_DMG_MULT", ["%.1f" % diff["hp_mult"], "%.1f" % diff["dmg_mult"]])
		...
		reward_lbl.text = I18n.t("REWARD") + " ×%.1f" % diff["reward_mult"]
